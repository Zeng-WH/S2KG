...
from metric.evaluation import eval_bleu
import copy
import math
...
'''   
try:
    nltk.data.find("/home/hadoop-aipnlp/cephfs/data/zengweihao02/nltk_data/tokenizers/punkt")
...
def main():
    ...
    def compute_metrics(eval_preds):
        ...
        decoded_labels = [item.strip().replace(' ','') for item in decoded_labels]
        decoded_preds = [item.strip().replace(' ','') for item in decoded_preds]       
        result = eval_bleu(decoded_preds, decoded_labels)
        ...
        #eval_result = eval_user_intent(decoded_preds, decoded_labels)
    ...
    trainer.init_chn(max_length=max_length, num_beams=num_beams)   
    # Training
    if training_args.do_train:
        checkpoint = None
    ...
    if training_args.do_eval:
        ...
        bleu_scores = eval_bleu(predictions, refference_summaries) 
        #eval_result = eval_user_intent(predictions, refference_summaries)
        #print(metrics)
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                with open(data_args.test_file, 'r') as r:
                    for item in jsonlines.Reader(r):
                        refference_summaries.append(item["summary"])
                rouge_scores = compute_rouges(predictions, refference_summaries)
                #bleu_scores = compute_bleus(predictions, refference_summaries)
